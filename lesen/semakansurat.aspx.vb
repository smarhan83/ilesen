...
Partial Class semakansurat
    ...
    Protected Sub BT_ViewMail_Command(sender As Object, e As CommandEventArgs)
        ...
        ViewSuratPemeriksaan(pid, jid)

    End Sub


    Private Sub ViewSuratPemeriksaan(permohonanID As String, jenislesenID As Integer)
        ...
        Dim jenisLesenDesc = {"", "sp_perniagaan", "sp_pasar", "sp_anjing", "sp_penjaja", "sp_billboard", "sp_tukaralamat", "sp_tambahpremis", "sp_tambahjenis",
            "sp_tukarpemilik", "sp_tukariklan", "sp_tukarnama", "sp_kurangiklan", "sp_kakilima", "sp_batal", "sp_ekspo"}
        ...
        Try
            sql = "SELECT a.KadarBayaran_Desc, a.KadarBayaran_Amount, b.*, c.Pemohon_Name, c.Pemohon_Address, c.Pemohon_ICNo, c.Pemohon_MobileNo, c.Pemohon_TelNo, d.Users_Fullname, d.Users_Signature, e.name AS AnjingJenisPremisDesc " &
                    "FROM LESEN_KadarBayaran a FULL OUTER JOIN LESEN_Permohonan b ON a.KadarBayaran_PermohonanID = b.Permohonan_ID INNER JOIN LESEN_Pemohon c ON b.Permohonan_PemohonID = c.Pemohon_ID " &
                    "LEFT JOIN TBL_USERS d ON b.TandatanganPemeriksaanId = d.Users_Id LEFT JOIN TBL_LOOKUPS e ON e.id = b.AnjingJenisPremis " &
    ...
    Protected Sub lbSurat_Click(sender As Object, e As EventArgs) Handles lbSurat.Click
        ...
        ViewSuratMohon(GridView1.SelectedDataKey.Values(0), GridView1.SelectedDataKey.Values(1), isBatal)
    End Sub

    Private Sub ViewSuratMohon(permohonanAgensiID As String, jenislesenID As Integer, isBatal As Boolean)
